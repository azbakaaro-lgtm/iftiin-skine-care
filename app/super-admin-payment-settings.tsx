...
export default function SuperAdminPaymentSettingsScreen() {
  return <RequireRole role="super_admin"><SuperAdminPaymentSettingsContent /></RequireRole>;
}

function SuperAdminPaymentSettingsContent() {
  const { session } = usePhaseSession();
  const settings = trpc.phase1.superPaymentSettings.useQuery({ sessionToken: session!.token });
  const save = trpc.phase1.saveSuperPaymentSettings.useMutation();
  ...
  const noticeStyle = notice.startsWith("Akoonka Iftiin") ? styles.success : styles.error;
  ...
  async function submit() {
    ...
    try {
      ...
      setNotice("Akoonka Iftiin waa la kaydiyey.");
    } catch (issue) {
      setNotice(issue instanceof Error ? issue.message : "Akoonka lama kaydin karo.");
    }
  }

  return <Page><ScrollView contentContainerStyle={styles.scroll}>
    <Header title="Xisaabta Iftiin" subtitle="Akoonka lagu heli karo commission-ka Iftiin" back={() => router.back()} />
    <Card style={styles.hero}>
      <View style={styles.heroIcon}><MaterialIcons name="account-balance-wallet" size={27} color={palette.purple} /></View>
      <View style={styles.heroCopy}><Text style={styles.heroTitle}>Akoonka commission-ka</Text><Text style={styles.heroText}>Kani waa akoonka Maamulaha Sare ee loogu talagalay in lagu helo 5% commission-ka dalabyada la xaqiijiyey.</Text></View>
    </Card>
    <Card style={styles.form}>
      ...
      <PrimaryButton label={save.isPending ? "Waa la kaydinayaa..." : "Kaydi akoonka Iftiin"} icon="save" onPress={submit} disabled={save.isPending} />
    </Card>
  </ScrollView></Page>;
}
