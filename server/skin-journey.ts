...
export async function unlockSkinJourneyForOrder(customerId: number, orderId: number) {
  const db = await getDb();
  if (!db) throw new Error("Kaydka xogta lama heli karo hadda.");
  await db.update(customerSkinJourneys).set({ status: "unlocked", unlockedAt: new Date() }).where(and(eq(customerSkinJourneys.customerId, customerId), eq(customerSkinJourneys.purchaseOrderId, orderId), eq(customerSkinJourneys.status, "preview")));
}
