...
const productInput = z.object({ name: z.string().min(2).max(180), brand: z.string().min(2).max(160), category: z.string().min(2).max(100), description: z.string().max(3000).optional().nullable(), usageInstructions: z.string().max(3000).optional().nullable(), originalPrice: z.number().int().min(0), discountType: z.enum(["none", "percentage", "fixed"]), discountValue: z.number().int().min(0), stock: z.number().int().min(0), availability: z.boolean(), imageData: z.string().regex(/^data:image\/(jpeg|jpg|png);base64,/).max(1_200_000).optional().nullable(), additionalImagesData: z.array(z.string().regex(/^data:image\/(jpeg|jpg|png);base64,/).max(1_200_000)).max(4).optional().nullable(), keepAdditionalImages: z.array(z.string()).max(4).optional().nullable() });
...
export const appRouter = router({
  ...
  phase1: router({
    ...
    scanProductLabel: publicProcedure.input(tokenInput.extend({ labelImage: z.string().regex(/^data:image\/(jpeg|jpg|png);base64,/).max(1_200_000) })).mutation(async ({ input }) => { await roleOrError(input.sessionToken, "store_admin"); return scanProductLabel(input.labelImage); }),
